...
import com.eddy.base.base.BaseFragment;
import com.eddy.base.base.BaseSwipeBackActivity;
...
public class SwipeBackLayout extends FrameLayout {
    ...
    class ViewDragCallback extends ViewDragHelper.Callback {
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            ...
            if (mScrollPercent > 1) {
                if (mFragment != null) {
                    if (mPreFragment instanceof BaseFragment) {
                        ((BaseFragment) mPreFragment).setLocking(true);
                    }
                    if (!mFragment.isDetached()) {
                        mFragment.setLocking(true);
                        FragmentManager fragmentManager = mFragment.getFragmentManager();
                        if (null != fragmentManager) {
                            fragmentManager.popBackStackImmediate();
                        }
                        mFragment.setLocking(false);
                    }
                    if (mPreFragment instanceof BaseFragment) {
                        ((BaseFragment) mPreFragment).setLocking(false);
                    }
                } else {
        ...
        public int getViewHorizontalDragRange(View child) {
            if (mFragment != null) {
                return 1;
                    ...
                    && (mActivity instanceof BaseSwipeBackActivity)
                    && ((BaseSwipeBackActivity) mActivity).swipeBackPriority()) {
                return 1;
            }
            return 0;
        }
